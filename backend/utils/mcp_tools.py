...
def get_mcp_status_in_apps(package_name: str) -> Dict[str, Any]:
    """获取指定MCP在所有智能体应用中的配置状态"""
    try:
        cmd = ["/var/lib/dev-store/src/mcp_manage.sh", "mcp-status", package_name]
        ...
        if returncode == 0:  # 成功执行
            ...
            return {
                "is_success": True,
                "data": app_status_list,
                "message": "Query successful",
                "status_code": status.HTTP_200_OK
            }
        else:  # 执行失败
            error_msg = stderr or stdout or "Query failed"
            return {
                "is_success": False,
                "data": [],
                "message": error_msg,
...
def manage_mcp_config(action: str, package_name: str, app_name: str) -> Dict[str, Any]:
    """管理MCP配置（添加或删除）"""
    ...
    try:
        ...
        cmd = ["/var/lib/dev-store/src/mcp_manage.sh", script_cmd, package_name, app_name]
        executor = CommandExecutor(cmd, timeout=30)
        stdout, stderr, returncode = executor.run()
